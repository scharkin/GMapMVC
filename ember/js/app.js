...
App.Cafe = Ember.Object.extend({});

App.CafeController = Ember.ObjectController.extend({
  selected: false,
  ...
  selectCafe: function () {
    var cafes = this.parentController._subControllers;
    for (var i = cafes.length-1; i >= 0; i--) {
      if(cafes[i].get("selected")) {
        cafes[i].set("selected", false)
      }
    }
...
App.MapView = Ember.CollectionView.extend({
  ...
  contentBinding: 'App.CafesController',

  didInsertElement: function () {
    var mapOptions = {
      center: new google.maps.LatLng(42.33, -71.1),
      zoom: 8
    };
    this.map = new google.maps.Map(document.getElementById(this.mapTagId), mapOptions);
  }
});
...
App.MarkerView = Ember.View.extend({
  ...
  observeSelected: function () {
    ...
    this.get("parentView").get("map").panTo(this.marker.position);
    setTimeout(function() {
